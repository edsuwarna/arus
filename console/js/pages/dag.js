/* DAG View Page — 3-layer asset graph (Source → Staging → Analytics) */
let dagState = { panX: 0, panY: 0, zoom: 1 };
const DAG_LAYERS = ['source', 'staging', 'analytics'];
const DAG_LABELS = { source: 'SOURCE LAYER', staging: 'STAGING LAYER', analytics: 'ANALYTICS LAYER' };
const LAYER_COLORS = { source: '#3b82f6', staging: '#f59e0b', analytics: '#10b981' };

async function renderDagPage(container) {
    container.innerHTML = `<div class="loading"><div class="spinner"></div><p>Loading DAG data...</p></div>`;
        ...
        container.innerHTML = `
            <div class="page-header">
                <div>
                    ...
                    <p class="page-subtitle">Pipeline asset graph — ${pipelines.length} pipelines</p>
                </div>
                <div style="display:flex;gap:8px">
                    <button class="btn btn-secondary btn-sm" onclick="resetDagView()">⟲ Auto-layout</button>
            ...
            ${pipelines.length ? `
            <div class="dag-pipeline-selector">
                ${pipelines.map(p => `
                    <div class="dag-pipeline-card ${p.last_run_status === 'failed' ? 'degraded' : ''}" onclick="showDagPipeline('${p.id}')">
                        <div class="dag-pipeline-name">${p.name}</div>
                        <div class="dag-pipeline-meta">
                            <span class="badge badge-${p.last_run_status === 'success' || p.last_run_status === 'not_started' ? 'success' : p.last_run_status === 'failed' ? 'danger' : 'info'}">
                                <span class="status-dot ${p.last_run_status === 'success' ? 'success' : p.last_run_status === 'failed' ? 'failed' : 'running'}"></span>
                                ${p.table_count} assets
                            </span>
                        </div>
                    </div>
                `).join('')}
            ...
            <div class="card dag-graph-container">
                <div class="dag-graph-toolbar">
                    <span style="font-size:13px;font-weight:500" id="dag-pipeline-label">All Pipelines</span>
            ...
            <div class="empty-state">
                ...
                <p>No pipeline data to visualize. Create a pipeline first.</p>
            </div>
            `}

            <!-- Pipeline detail bottom panel -->
...
function renderDagGraph(pipelines) {
    ...
    const layers = { source: [], staging: [], analytics: [] };
    const allEdges = [];
    const seen = { source: new Set(), staging: new Set(), analytics: new Set() };
    ...
    const nodePositions = {};
    ['source', 'staging', 'analytics'].forEach(layer => {
    ...
    const layerLabels = {
        ...
        staging: { x: layerX.staging, label: 'STAGING LAYER', color: '#f59e0b' },
        analytics: { x: layerX.analytics, label: 'ANALYTICS LAYER', color: '#10b981' },
    };
    ...
    allEdges.forEach(e => {
        ...
        if (from && to) {
            ...
            const cx = (x1 + x2) / 2;
            html += `<path d="M${x1},${y1} C${cx},${y1} ${cx},${y2} ${x2},${y2}" stroke="#2a2d35" stroke-width="1.5" fill="none" opacity="0.6"/>`;
        }
    });
    ...
    Object.entries(nodePositions).forEach(([name, pos]) => {
        ...
        html += `<g class="dag-node" onclick="dagNodeClick('${name}')" style="cursor:pointer">
            <rect x="${pos.x}" y="${pos.y}" width="${NODE_W}" height="${NODE_H}" rx="6"
                  ...
                  stroke="${isSelected ? color : '#23262e'}"
        ...
        panel.innerHTML = `
            <div class="card" style="margin-top:16px">
                ...
                <div style="display:flex;gap:16px;margin:12px 0">
                    <div><span style="color:var(--text-muted);font-size:11px">LAYER</span><br><span style="font-size:13px;text-transform:uppercase">${layer === 'source' ? '🔵' : layer === 'staging' ? '🟠' : '🟢'} ${layer}</span></div>
